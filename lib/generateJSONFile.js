...
try {
  // 初始化时该文件还不存在
  siteConfig = require(path.join(process.cwd(), './site_config/site.js')).default;
} catch (err) {
  // do noting
}
  ...
  const parse = dir => {
    ...
    files.forEach(file => {
      ...
      if (stat.isFile() && (extension === '.md' || extension === '.markdown')) {
        ...
        if (
          type === 'docs' &&
          fs.existsSync(path.join(cwd, './src/pages/documentation/index.jsx'))
        ) {
          const Documentation = require(path.join(cwd, './src/pages/documentation/index.jsx'))
            .default;
          ejs.renderFile(
          ...
          type === 'blog' &&
          fs.existsSync(path.join(cwd, './src/pages/blogDetail/index.jsx'))
        ) {
          const BlogDetail = require(path.join(cwd, './src/pages/blogDetail/index.jsx')).default;
          ejs.renderFile(
            path.join(cwd, './template.ejs'),
            {
              title: result.meta.title || fileInfo.name,
